...
func (c *Client) refreshHostIDListCache(rid string) error {
	...
	for {
		...
		pip.Expire(ctx, tempIDListKey, time.Duration(hostKey.HostIDListKeyExpireSeconds())*time.Second)
		for _, h := range stepID {
			...
			pip.ZAdd(ctx, tempIDListKey, key)
		}
	...
	pipe.Rename(ctx, hostKey.HostIDListKey(), tempOldIDListKey)
	// rename temp key to real key
	pipe.Rename(ctx, tempIDListKey, hostKey.HostIDListKey())
	// reset id_list key's expire time to a new one.
	pipe.Expire(ctx, hostKey.HostIDListKey(), 48*time.Hour)
	// set expire key with unix time seconds now value.
	pipe.Set(ctx, hostKey.HostIDListExpireKey(), time.Now().Unix(),
		time.Duration(hostKey.HostIDListKeyExpireSeconds())*time.Second)
